	...
	function Postbox($replythread = 0) {
		...
		if ($this->board['enablecaptcha'] ==  1)
		{
			...
			else if( KU_CAPTCHA_TYPE == 'faptcha' )
			{
				...
				// 06/01/2012 SRC now also appends random querystring in case of caching issues
				// 15/01/2012 blank answer textbox on refresh to avoid accidental wrong answers
				$faptcha_image = '<a href="#" onclick="javascript:document.getElementById(\'captchaimage\').src = \''. KU_CGIPATH . '/faptcha.php?\' + Math.random();javascript:document.getElementById(\'captcha\').value=\'\';return false;"> <IMG ID="captchaimage" SRC="'. KU_WEBFOLDER .'faptcha.php?'.mt_rand().'" border="0" alt="Click for a new faptcha" title="Click for a new faptcha"> </A>';
				$faptcha_input = '<BR><input type="text" name="captcha" id="captcha" title="Enter character\'s name (common nicknames accepted ... probably)" size="28" maxlength="75" accesskey="c" />';
...
class Post extends Board {
	...
	function Insert($parentid, $name, $tripcode, $email, $subject, $message, $filename, $file_original, $filetype, $file_md5, $image_w, $image_h, $filesize, $thumb_w, $thumb_h, $password, $timestamp, $bumped, $ip, $posterauthority, $tag, $stickied, $locked, $boardid) {
		...
		// RH - ipmd5 now salted (insecure poster ID issue)
		...
		if(!$id || KU_DBTYPE == 'sqlite') {
			...
			$id = $tc_db->GetOne("SELECT `id` FROM `".KU_DBPREFIX."posts` WHERE `boardid` = ".$tc_db->qstr($boardid)." AND timestamp = ".$tc_db->qstr($timestamp)." AND `ipmd5` = '".md5(KU_SALT.$ip)."' LIMIT 1");
		}
