 ...
 * ONEECHAN 1.0.1
 *
 * ONEECHAN IS BASED ON KUSABA X 0.9.2
 *
...
/* Add, edit, delete, and view news entries */
	function news() {
      ...
      <input type="hidden" name="token" value="' . $_SESSION['token'] . '" />
		...
		global $tpl_page;
		$this->AdministratorsOnly();
		$faq = KU_ROOTDIR . 'rules.php';
		if (!empty($_POST['niggers'])) {
			file_put_contents($faq, $_POST['niggers']);
			$tpl_page .= '<hr />'. _gettext('Templates successfully edited.') .'<hr />';
		}
		$content = file_get_contents(KU_ROOTDIR . 'rules.php');

		$tpl_page .= '<h2>'. _gettext('Frequently Asked Questions') .'</h2> Raw PHP editor for the Rules page.<br /><br />'. "\n" .
