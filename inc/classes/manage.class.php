...
/* Add, edit, delete, and view news entries */
    ...
    function proxyban() {
        ...
        if (isset($_FILES['imagefile'])) {
            ...
            # Ban time function added by Courtney
            $bantime = $_POST['btime'];
    ...
    */
    ...
    function del_rs() {
        global $tc_db, $tpl_page;
        $this->ModeratorsOnly();
        $tpl_page .="<h2>Rapidshare Board Management</h2>";
        // If the file to delete is set, delete it right away,
        if(isset($_GET['del'])) {
            $tc_db->Execute("DELETE FROM `".KU_DBPREFIX."rs_links` WHERE `id` = ".$tc_db->qstr($_GET['del'])." LIMIT 1");
            $tpl_page .="Download ". $_GET['del'] . " removed.";
        } elseif (isset($_POST['del'])) {
    ...
    function bans() {
		...
		$banReasons = unserialize(TSUKI_BANREASONS);
		foreach ($banReasons as $key => $value) {
			$tpl_page .= '<a href="#" onclick="document.banform.reason.value=\''. _gettext($value) .'\';return false;">' . _gettext($key) . '</a>&nbsp;';
		}
		...
		foreach ($banReasons as $key => $value) {
			$tpl_page .= '<a href="#" onclick="document.banform.staffnote.value=\''. _gettext($value) .'\';return false;">' . _gettext($key) . '</a>&nbsp;';
		}
		
		$tpl_page .= '|| '. _gettext('This message will be shown only on this page and only to staff, not to the user.') .'</div><br />';
        ...
        <span style="color:red"><b>ATTENTION!  If you ban a spammer, also filter their spam in <a href="manage_page.php?action=spam">Block & Ban Content</a>!!!</span>
        <hr /><br />';
...
print "<form method=\"POST\" action=\"manage_page.php?action=proxycheck\">
...
@list($proxy, $type, $country) = explode("|", $line);
    ...
    {
    $fontcolor = $celite;
    $handle = fopen(KU_ROOTDIR . $goodfilename, "a");
...
print "</table>";
    ...
    */
    ...
    function spam() {
      ...
      $this->CheckToken($_POST['token']);
            ...
            $tpl_page .= '<hr />'. _gettext('Content filters successfully edited.') .'<hr />';
        }
        $content = htmlspecialchars(file_get_contents(KU_ROOTDIR . 'spam.txt'));

        $tpl_page .= '<h2>'. _gettext('Block &amp; Ban Content') .'</h2> If a post matches any line on this list, it will be blocked from posting and the user banned for one hour.<br />'. "\n" .
    ...
    # BLOCKFILTER - New feature in Hurrchan 1.0.x
        global $tpl_page;
        $spam = KU_ROOTDIR . 'block.txt';
        if (!empty($_POST['spam'])) {
            file_put_contents($spam, $_POST['spam']);
            $tpl_page .= '<hr />'. _gettext('Content filters successfully edited.') .'<hr />';
        }
        $content = htmlspecialchars(file_get_contents(KU_ROOTDIR . 'block.txt'));

		$tpl_page .= '<h2>'. _gettext('Block Content') .'</h2> If a post matches any line on this list, it will be blocked from posting and the user will be given a post failure warning.<br />'. "\n" .
					'<form action="?action=blockfilter" method="post">'. "\n" .
					'<textarea name="spam" rows="25" cols="80">' . htmlspecialchars($content) . '</textarea><br />' . "\n" .
					'<input type="submit" value="'. _gettext('Submit') .'" />'. "\n" .
					'</form>'. "\n";
	}
	...
	*/
		...
		$tpl_page .= '<h2>'. _gettext('Search Chan Spam Blacklist') .'</h2> Search for records of an IP address in the chan spam blacklist.<br />'. "\n" .
					...
					'<input type="hidden" name="addchan" value='. KU_NAME .'><input type="hidden" name="adduser" value='. $_SESSION['manageusername'] .'>'. "\n" .
					'<td><input type="text" name="comment"></td><td><input type="text" name="spam"></td><td><input type="submit" name="Add"></tr></table>'. "\n" .
					'Note: All fields are required for a new entry.</form>'. "\n";
	}
	...
	function editpost() {
	global $tc_db, $tpl_page;
	$this->ModeratorsOnly(); /*OR $this->AdministratorsOnly(); */ /*Select whether you want this option was for modetartors or only administators */
	...
	$results = $tc_db->GetAll("SELECT HIGH_PRIORITY `parentid`,`message` FROM `" . KU_DBPREFIX . "posts` WHERE `boardid` = " . $board_id . " AND `id` = " . $tc_db->qstr($editpostid) . " ");
