...
function formatDisplayName($name, $tripcode, $anon = 'Anonymous') {
 ...
 */
function formatLongMessage($message, $board, $threadid, $page, $maxLines = 15, $maxLength = KU_LINELENGTH) {
	$output = '';
	if ((strlen($message) > $maxLength || count(explode('<br />', $message)) > $maxLines) && $page) {
		...
		if (strlen($message_shortened) > $maxLength) {
			$message_shortened = substr($message_shortened, 0, $maxLength);
		}
...
function getPostUrl($post_board, $post_thread_start_id, $post_id) {

	return KU_BOARDSFOLDER . $post_board . '/res/' . $post_thread_start_id . '.html#' . $post_id;
