...
if (!$cache_loaded) {
	...
	// Robot9000
		$cf['ROBOT_ENABLE'] = false;			// Enable robot (duplicate post detection)
		$cf['ROBOT_STRIP_REPEATING'] = true;	// Strip repeating characters
		$cf['ROBOT_STRIP_HTML'] = true;			// Strip HTML tags (BBCode basically)
		$cf['ROBOT_STRIP_QUOTES'] = true;		// Strip quotes (">" and ">>")
		$cf['ROBOT_MIN_POST_CHARS'] = 3;		// Minimum number of characters in a post
		$cf['ROBOT_MUTE'] = true;				// If true, duplicate posts will cause a temporary (ever-increasing) mute
