...
function removeframes()
{
  ...
  for (var i = 0; i < a.length; i++) if (a[i].className == "boardlink") a[i].target = "_top";
...
function highlight(a, b)
{
  ...
  for (var i = 0; i < c.length; i++) if (c[i].className == "highlight") c[i].className = "reply";
...
function togglePassword() {
	// RH 08/01/2012 - improved (probably) browser testing, old way was breaking mod password box on Chrome occasionally - issue #11. I.e. assume innerHTML works properly unless we know otherwise.
	// I *assume* this is done to work around an MSIE quirk something like http://www.quirksmode.org/bugreports/archives/2004/11/innerhtml_and_t.html , partly due to the "Now IE/Opera safe"
	// comment in the kusaba X 0.9.3 version.
	var MSIE = (navigator.userAgent.indexOf('Microsoft Internet Explorer') != -1);
      ...
      if (d.getAttribute('id').substr(0, 3) == 'dnb')
      {
        ...
        new Ajax.Request(ku_boardspath + "/manage_page.php?action=getip&boarddir=" + e[1] + "&id=" + e[2], {
          ...
          onSuccess: function (a)
          {
            ...
            span = document.getElementById(ipaddr[0].replace(/^\s+/,""));
        ...
        f += 'id=' + e[2] + '&postid=' + e[2] + '&cp=y" title="' + _('Child Pornography') + '" onclick="return confirm(_(\'Are you sure that this is child pornography?\'));">CP<\/a>&#93;<a href="' + ku_boardspath + '/manage_page.php?action=editpost&boarddir=' + e[1] + '&editpostid=' + e[2] + '" target="_blank" title="Edit post">[Staff Edit]</a>';
        c[i].innerHTML = f
      }
    }
  }
...
function checkcaptcha(a)
{
  ...
  {
    if( document.getElementById(a).modpassword && document.getElementById(a).modpassword.value != '' )  // RH - mods/admins bypass faptcha if using posting pass
    {
	return true;
    }
    if (document.getElementById(a).captcha)
    {
      ...
      {
        alert('Please name the character in the faptcha (or click it for a new one)');
...
$(document).ready(function(){
	// RH - jQuery hook, called on page load.
	// Use this to apply FABULOUS colour-cycling links. Adapted from 7chan (apparently a custom feature?)
	if (title == 'Dorasfutaba') {	// Only active for "Dorasfutaba" CSS, as used on oneechan.org. Remove this to make it global
...
kusaba.colors = function() {	// RH - part of the colour-cycling links thing
    $(anchorobj).css("color", kusaba.makeColor())
}

kusaba.makeColor = function() {	// RH - part of the colour-cycling links thing
